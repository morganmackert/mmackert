...
#Filter Bees to be without target bees, wasps, or unidentifiable specimens
bees <- Bees %>%
  filter(Family != "Wasp") %>%
  filter(Family != "Fly") %>%
...
#Reformat from long to wide with Binomial as columns
beespp.site.wide <- spread(beespp.site, Binomial, n)

#Convert to a dataframe
beespp.site.wide <- as.data.frame(beespp.site.wide)

#Fill NAs with 0
beespp.site.wide[is.na(beespp.site.wide)] <- 0

#Change row names to Date.Site name
beespp.site.wide <- beespp.site.wide %>%
  column_to_rownames("Site")

#Estimate Inverse Simpson's Diversity Index for bee species richness
inv.simp <- diversity(beespp.site.wide, "inv")

#Move rownames to "Site" variable
inv.simp <- as.data.frame(inv.simp)
inv.simp <- rownames_to_column(inv.simp, var = "Site")

#Chao1 Richness Index
#Reformat from long to wide with Site as columns
beespp.site.long <- spread(beespp.site, Site, n)

#Fill NAs with 0
beespp.site.long[is.na(beespp.site.long)] <- 0

#Chao1 richness estimator
chao1 <- estimateR(beespp.site.wide)

#Export chao1 result as .csv
write.csv(chao1, file = "C:/Users/Morgan Mackert/Documents/ISU/Project/mmackert/Graphs/RichnessEstimates/Chao1 Richness Estimates 12345.csv")
